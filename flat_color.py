...
def flat_color_multi_scale(image_path, n_colors=20, scales=[1.0, 0.5, 0.25], 
    ...
    # Post-processing - Enhanced smoothing for high color counts
    ...
    out_path = image_path.replace(".", "_flatcolor_multiscale.")
    cv2.imwrite(out_path, cv2.cvtColor(out_img, cv2.COLOR_RGB2BGR))
    print(f"Multi-scale flat color image saved: {out_path}")
    return out_path

if __name__ == "__main__":
    start_time = time.time()
    # Multi-scale version - Optimized parameters for n_colors=512
    flat_color_multi_scale("ai.png", 
                          n_colors=2048,        # High color count
                          temperature=2,        # Higher temperature to enhance soft assignment
                          spatial_scale=80,     # Larger spatial weight to promote large region formation
                          sharpen_strength=0,   # Sharpening strength
                          block_size=512
                          )
    end_time = time.time()
